...
- **REST API support via gRPC-Gateway** (in addition to gRPC)
...
In addition to gRPC, the emulator also supports REST/HTTP API via [gRPC-Gateway](https://github.com/grpc-ecosystem/grpc-gateway). This allows you to interact with the emulator using standard HTTP/JSON requests, similar to the official Google Cloud Tasks REST API.

### Enabling REST API

To enable the REST API, specify the `-http-port` flag:

```sh
go run ./ -host localhost -port 8123 -http-port 8124
```

This will start:
- gRPC server on `localhost:8123`
- REST/HTTP server on `localhost:8124`

### Docker with REST API

```sh
docker run -p 8123:8123 -p 8124:8124 tasks_emulator -host 0.0.0.0 -port 8123 -http-port 8124
```
...
gcloud-tasks-emulator:
  ...
  command: -host 0.0.0.0 -port 8123 -http-port 8124 -queue "projects/dev/locations/here/queues/anotherq"
  ports:
    - "${TASKS_PORT:-8123}:8123"
    - "${TASKS_HTTP_PORT:-8124}:8124"
  environment:
...
The REST API follows the [Cloud Tasks API v2](https://cloud.google.com/tasks/docs/reference/rest) specification.

**List Queues:**
```sh
curl http://localhost:8124/v2/projects/my-project/locations/us-central1/queues
```

**Create a Queue:**
```sh
curl -X POST http://localhost:8124/v2/projects/my-project/locations/us-central1/queues \
  -H "Content-Type: application/json" \
  -d '{"name": "projects/my-project/locations/us-central1/queues/my-queue"}'
```

**Get a Queue:**
```sh
curl http://localhost:8124/v2/projects/my-project/locations/us-central1/queues/my-queue
```

**Create a Task:**
```sh
curl -X POST http://localhost:8124/v2/projects/my-project/locations/us-central1/queues/my-queue/tasks \
  -H "Content-Type: application/json" \
  -d '{
    "task": {
      "httpRequest": {
...
**List Tasks:**
```sh
curl http://localhost:8124/v2/projects/my-project/locations/us-central1/queues/my-queue/tasks
```

**Delete a Queue:**
```sh
curl -X DELETE http://localhost:8124/v2/projects/my-project/locations/us-central1/queues/my-queue
```

**Pause a Queue:**
```sh
curl -X POST http://localhost:8124/v2/projects/my-project/locations/us-central1/queues/my-queue:pause \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
**Resume a Queue:**
```sh
curl -X POST http://localhost:8124/v2/projects/my-project/locations/us-central1/queues/my-queue:resume \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
You can now use any REST client library to interact with the emulator. For example, if you have a package that builds REST clients for GCP APIs, you can point it to the emulator's REST endpoint.
...
EMULATOR_URL = "http://localhost:8124"
PROJECT = "my-project"
...
response = requests.post(
    f"{EMULATOR_URL}/v2/projects/{PROJECT}/locations/{LOCATION}/queues",
    json={"name": queue_path}
)
print(response.json())

# Create a task
task_body = base64.b64encode(json.dumps({"message": "hello"}).encode()).decode()
response = requests.post(
    f"{EMULATOR_URL}/v2/{queue_path}/tasks",
    json={
        "task": {
            "httpRequest": {
                "url": "https://example.com/handler",
...
### Docker image
Or even easier - pull and run it directly from Docker Hub:
```sh
docker pull harshcasper/cloud-tasks-emulator:latest
docker run -p 8123:8123 harshcasper/cloud-tasks-emulator:latest -host 0.0.0.0 -port 8123
```

With REST API support:
```sh
docker run -p 8123:8123 -p 8124:8124 harshcasper/cloud-tasks-emulator:latest \
  -host 0.0.0.0 -port 8123 -http-port 8124 \
  -queue projects/dev/locations/here/queues/anotherq
```
...
```yml
gcloud-tasks-emulator:
  image: harshcasper/cloud-tasks-emulator:latest
  command: -host 0.0.0.0 -port 8123 -http-port 8124 -queue "projects/dev/locations/here/queues/anotherq"
  ports:
    - "${TASKS_PORT:-8123}:8123"
    - "${TASKS_HTTP_PORT:-8124}:8124"
